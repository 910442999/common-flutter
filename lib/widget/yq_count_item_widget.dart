...
/// 显示数量子控件 (购物车)
class YQCountItemWidget extends StatefulWidget {
  /// 数量
  int count;
  ...
  _YQCountItemWidgetState createState() => _YQCountItemWidgetState();
}

class _YQCountItemWidgetState extends State<YQCountItemWidget> {
  /// 数量
  late int _count;
  String? toastContent;
  ...
  Widget _rightNumber() {
    return GestureDetector(
      ...
      onTap: () {
        if (!widget.isEnable) {
          print("禁止响应");
          if (toastContent != null) YQToastUtils.show(toastContent!);
          return;
        }
        setState(() {
          this._count++;
