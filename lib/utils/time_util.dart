...
class TimeUtil {
    ...
    if (nowTime.year == msgTime.year) {
      ...
      if (nowTime.month == msgTime.month) {
        ...
        if (nowTime.day == msgTime.day) {
          //同一天 时:分
          return DateFormat(DateFormats.h_m).format(msgTime).toString();
        } else {
        ...
        return DateFormat(DateFormats.mo_d_h_m).format(msgTime).toString();
      }
    }
    return DateFormat(DateFormats.y_mo_d).format(msgTime).toString();
  }
  ...
  static String timestampConvertTime5(int timestamp) {
    ...
    if (timestamp.abs() > 60 * 60 * 1000) {
      return DateFormat(DateFormats.h_m_s).format(msgTime).toString();
    } else {
      return DateFormat(DateFormats.m_s).format(msgTime).toString();
    }
  }
...
//获取年龄
  static String getAge(String? time) {
    if (TextUtil.isEmpty(time)) return "0";
    int age = 0;
